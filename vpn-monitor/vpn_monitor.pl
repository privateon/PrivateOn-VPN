...
sub fake_systemv_logger
{
	...
	if ( system('pidof systemd-journald') eq 0 ) {
		...
		system("/opt/PrivateOn/vpn-monitor/vpn_logger.sh &");
		return 0;
	}
...
ROUTE_DEL:
	...
	if (!$status) {
		...
		# start httpd here
		system("thttpd -r -h localhost -d /opt/PrivateOn/vpn-monitor/htdocs");
	} else {
...
sub run_once {
	...
	if ( !-e STATUS_FILE) {
		...
		my $vpn_ini;
		unless (open $vpn_ini, "</opt/PrivateOn/vpn-default.ini") {
			$ctx->log(error => "Could not open /opt/PrivateOn/vpn-default.ini  Reason: " . $!);
			if (defined $ARGV[0]) { spawn_undo_crippling() };
			return -1;
		}
